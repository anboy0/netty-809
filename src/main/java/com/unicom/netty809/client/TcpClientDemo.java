...
import com.unicom.netty809.vomodel.UpExgMsgRegisterEntity;
...
//    }
     ...
     * @return 2016年9月28日 by fox_mt
     */
    public boolean sendMsg2Gov(Object obj) {
        boolean success = false;
        if (isLogined()) {
            //已经登录成功，开始发送数据
            channel = tcpClient.getChannel(Constants.TCP_ADDRESS_FUJIAN, Constants.TCP_RESULT_PORT);
            if (null != channel && channel.isWritable()) {
                if (obj instanceof UpExgMsgRegisterEntity) {
                    UpExgMsgRegisterEntity entity = (UpExgMsgRegisterEntity)obj;
                    Message msg = buildSendRegisterVO(entity);
                    channel.write(msg);
                    LOGGER.info("发送车辆注册信息--" + entity.toString());
                } else if(obj instanceof UpExgMsgRealLocationEntity){
                    UpExgMsgRealLocationEntity awsVo = (UpExgMsgRealLocationEntity)obj;
                    Message msg = buildSendVO(awsVo);
                    channel.write(msg);
                    LOGGER.info("发送实时定位信息--" + awsVo.toString());
                }
            } else {
                LONGINSTATUS = "";
            }
     ...
     * @param awsVo 2016年9月28日 by fox_mt
     */
    private Message buildSendRegisterVO(UpExgMsgRegisterEntity entity) {
        Message msg = new Message(JT809Constants.UP_EXG_MSG);
        ChannelBuffer buffer = ChannelBuffers.buffer( 77);//数据体总长
        buffer.writeBytes(getBytesWithLengthAfter(21, entity.getVehicleNo().getBytes(Charset.forName("GBK"))));//21
        buffer.writeByte(1);//1
        buffer.writeShort(JT809Constants.UP_EXG_MSG_REGISTER);//2
        buffer.writeInt(49);//4
        //平台唯一编码
        buffer.writeBytes(getBytesWithLengthAfter(11,entity.getPlatformId().getBytes(Charset.forName("GBK"))));// 11
        //车载终端厂商唯一编码
        buffer.writeBytes(getBytesWithLengthAfter(11,entity.getProducerId().getBytes(Charset.forName("GBK"))));// 11
        //车载终端型号
        buffer.writeBytes(getBytesWithLengthAfter(8,entity.getTerminalModelType().getBytes(Charset.forName("GBK"))));// 8
        //车载终端编号
        buffer.writeBytes(getBytesWithLengthAfter(7,entity.getTerminalId().getBytes(Charset.forName("GBK"))));// 7
        //车载终端SIM卡电话号码
        buffer.writeBytes(getBytesWithLengthAfter(12,entity.getTerminalSimCode().getBytes(Charset.forName("GBK"))));// 12
        msg.setMsgBody(buffer);
        return msg;
    }
    ...
    public static void main(String[] args) {
        ...
        UpExgMsgRegisterEntity entity = new UpExgMsgRegisterEntity();
        entity.setVehicleNo("浙000001");
        entity.setPlatformId("111");
        entity.setProducerId("111");
        entity.setTerminalModelType("111");
        entity.setTerminalId("111");
        entity.setTerminalSimCode("13211111111");
        s.sendMsg2Gov(entity);
        try {
